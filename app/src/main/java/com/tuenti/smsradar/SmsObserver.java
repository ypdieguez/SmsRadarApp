...
public class SmsObserver extends ContentObserver {
    ...
    private void processSms(Cursor cursor) {
        ...
        try {
            String protocol = cursor.getString(cursor.getColumnIndex(PROTOCOL_COLUMN_NAME));
            smsCursor = getSmsCursor(protocol);
            Sms sms = parseSms(smsCursor);

            if (sms != null) {
                if (sms.getType() == SmsType.SENT) {
                    smsRadarService.sendBroadcastAndNotify(
                            smsCursorParser.smsStorage.updateSmsSentCount(sms));
                }
            }
        } finally {
            close(smsCursor);
